// Mock API fallback — used when backend is unreachable
let isMock = false;
...
export async function mockFetch(path, options) {
    ...
    const url = path.replace('/api', '') || path;
    ...
    if (path === '/login' && method === 'POST') {
        ...
        if (method === 'POST') {
            const item = { id: Date.now(), title: body.title, description: body.description || '', deadline: body.deadline, completed: false, completed_at: null };
            list.push(item);
        ...
        if (method === 'PUT') {
            list = list.map(i => {
                if (i.id == body.id) {
                    return { ...i, completed: body.completed, completed_at: body.completed ? (i.completed_at || new Date().toISOString()) : null };
                }
                return i;
            });
            save('assignments', list);
        ...
        if (method === 'PUT') { save('anniversary', body); data = body; }
    }
    else if (path.startsWith('/goals')) {
        let goals = db('goals', []);
        if (method === 'GET') data = goals;
        if (method === 'POST') {
            const g = { id: Date.now(), title: body.title, deadline: body.deadline, progress: 0, category: body.category || 'Personal', completed: false };
            goals.push(g);
            save('goals', goals);
            data = g;
        }
        ...
        if (method === 'POST') {
            const e = { id: Date.now(), date: body.date, mood: body.mood, note: body.note, created_at: new Date().toISOString() };
            evals.push(e);
        ...
        if (method === 'POST') {
            const s = { id: Date.now(), day: body.day, start: body.start, end: body.end, subject: body.subject, room: body.room, lecturer: body.lecturer };
            schedule.push(s);
        ...
        const stats = {};
        if (!month) { status = 400; data = { error: 'Missing month' }; }
        else {
            ...
            users.forEach(u => {
                ...
                todos.forEach(t => {
                    ...
                    days.forEach(d => { if (d === prev + 1) cur++; else cur = 1; if (cur > max) max = cur; prev = d; });
                    if (max > maxUserStreak) maxUserStreak = max;
                });
                stats[u] = { completion_rate: totalPossible ? Math.round((totalCompleted / totalPossible) * 100) : 0, streak: maxUserStreak, total_completed: totalCompleted, total_possible: totalPossible };
            });
        ...
        if (method === 'GET') {
            if (!methodQSMonth || !methodQSUser) { status = 400; data = { error: 'Missing month or user' }; }
            else {
        ...
        if (method === 'POST') {
            ...
            if (action === 'create_todo') {
                ...
                if (!title || !month || !user_id) { status = 400; data = { error: 'Invalid data' }; }
                else {
                ...
                if (!affectedKey) { status = 404; data = { error: 'Todo not found' }; }
            } else { status = 400; data = { error: 'Unknown action' }; }
        }
        if (method === 'DELETE') {
            const id = qs.get('id');
            if (!id) { status = 400; data = { error: 'Missing id' }; }
            else {
                const users = ['Zaldy', 'Nesya'];
                let deleted = false;
                const currentMonth = new Date().toISOString().slice(0, 7);
                users.forEach(u => {
                    const key = `monthly_${currentMonth}_${u}`;
                    let todos = db(key, []);
                    const before = todos.length;
                    todos = todos.filter(t => t.id != id);
                    if (todos.length !== before) { save(key, todos); deleted = true; }
                });
    ...
    else { status = 404; data = { error: 'Not Found (Mock)' }; }
